...
})({"6SSHa":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
var _gameScore = require("./gameScore");
var _gameTile = require("./gameTile");
var _gameBoard = require("./gameBoard");
var _gameToken = require("./gameToken");
const customGameElements = {
    'game-board': _gameBoard.GameBoard,
    'game-token': _gameToken.GameToken,
    'game-tile': _gameTile.GameTile,
    'game-score': _gameScore.GameScore
};
const axes = {
    up: {
        main: 'x',
...
const board = document.getElementsByTagName('game-board')[0];
const score = document.getElementsByTagName('game-score')[0];
const tiles = Array.from(board.querySelectorAll('game-tile'));
...
function slideTokens(tiles1, direction) {
    const axisInfo = axes[direction];
    let lanes = sortTilesIntoLanes(tiles1, axisInfo);
    lanes.map((lane)=>{
        let tokenMerged = false;
        lane.forEach((tile, index)=>{
            const token = tile.querySelector('game-token');
            if (!token) return;
...
function sortTilesIntoLanes(tiles2, axisInfo) {
    const laneAxis = axisInfo.main;
    return tiles2.reduce((lanes, tile)=>{
        const lane = tile[laneAxis];
        if (lanes.hasOwnProperty(lane) && axisInfo.reversed) lanes[lane].unshift(tile);
        else if (lanes.hasOwnProperty(lane)) lanes[lane].push(tile);
        else lanes[lane] = [
            tile
        ];
        return lanes;
...
function slideSingleToken(index, lane, token, tokenMerged) {
    while(index > 0){
        ...
        const otherToken = tileToCheck.querySelector('game-token');
...
function isMovePossible() {
    ...
    return lanes.some((lane)=>lane.some((tile, index)=>{
            if (index + 1 === lane.length) return false;
            const token = tile.querySelector('game-token');
            const nextToken = lane[index + 1].querySelector('game-token');
            if (!(token && nextToken)) return false;
            else if (token.value === nextToken.value) return true;
        })
    );
}
...
function getEmptyTiles(tiles3) {
    return tiles3.filter((tile)=>Array.from(tile.children).every((child)=>!(child instanceof _gameToken.GameToken)
        )
    );
}
...
},{"./gameScore":"5JUK4","./gameTile":"ajQmF","./gameBoard":"8IN5w","./gameToken":"7TzNq","@parcel/transformer-js/src/esmodule-helpers.js":"7L7TQ"}],"5JUK4":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}],"ajQmF":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "GameTile", ()=>GameTile
);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"7L7TQ"}],"8IN5w":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"7L7TQ"}],"7TzNq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "GameToken", ()=>GameToken
);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"7L7TQ"}]},["6SSHa","4j3ZX"], "4j3ZX", "parcelRequire45ba")
